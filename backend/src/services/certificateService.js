...
const drawCertificatePage = (doc, event, user, organizer) => {
  // Set up document
...
const generateCertificate = async (event, user, organizer) => {
  return new Promise((resolve, reject) => {
    try {
      ...
      drawCertificatePage(doc, event, user, organizer);
...
const generateBulkCertificatesPdf = async (eventId) => {
  const Event = require('../models/Event');
  const event = await Event.findById(eventId).populate('organizers.userId');
  ...
  if (!event.certificatesGenerated) {
    throw new Error('Certificates have not been generated yet.');
  }
  ...
  return new Promise((resolve, reject) => {
    try {
      const doc = new PDFDocument({ size: [841.89, 595.28], margin: 50 });
      const chunks = [];
      doc.on('data', (chunk) => chunks.push(chunk));
      doc.on('end', () => resolve(Buffer.concat(chunks)));
      ...
      for (const attendee of attendedUsers) {
        ...
        if (!user) continue;
        if (pageIndex > 0) {
          doc.addPage({ size: [841.89, 595.28], margin: 50 });
        }
        drawCertificatePage(doc, event, user, organizer);
        pageIndex += 1;
      }
      ...
      if (pageIndex === 0) {
        throw new Error('No valid attendee records found for certificate export.');
      }
